...
class ReminderBase(BaseModel):
    ...
    reminder_time: datetime = Field(
        ..., description="Время, когда должно быть отправлено напоминание"
    )
...
class ReminderCreate(ReminderBase):
    ...
    is_sent: Optional[bool] = Field(
        False, description="Статус отправки напоминания (по умолчанию: False)"
    )
...
class ReminderUpdate(BaseModel):
    ...
    reminder_time: Optional[datetime] = Field(
        None, description="Новое время напоминания"
    )
    is_sent: Optional[bool] = Field(
        None, description="Обновленный статус отправки напоминания"
    )
